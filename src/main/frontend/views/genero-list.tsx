...
import { Button, Grid, GridColumn, TextField, VerticalLayout, Dialog } from '@vaadin/react-components'; 
  ...
  const createGenero = async () => {
    try {
      if (nombre.value.trim()) {
        await GeneroService.create(nombre.value);
...
export default function GeneroView() {
  const dataProvider = useDataProvider<any>({
    list: async () => {
      const result = await GeneroService.listGenero();
