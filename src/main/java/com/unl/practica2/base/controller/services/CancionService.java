...
public class CancionService {
    ...
    @NotEmpty String url, @NotEmpty String tipo, Integer idAlbum) throws Exception {
        ...
        tipo.trim().length() > 0 && duracion > 0 && idGenero > 0 && idAlbum > 0) {
            dc.getObj().setNombre(nombre);
            dc.getObj().setDuracion(duracion);
            dc.getObj().setIdAlbum(idAlbum);
            dc.getObj().setIdGenero(idGenero);
    ...
    public List<HashMap> listaAlbum() {
    ...
    public List<HashMap> listaGenero() {
        List<HashMap> lista = new ArrayList<>();
    ...
    public List<HashMap> listCancion() {
        ...
        if (!dc.listAll().isEmpty()) {
            ...
            for (int i = 0; i < arreglo.length; i++) {
                ...
                aux.put("idGenero", new DaoGenero().listAll().get(arreglo[i].getIdGenero() - 1).getNombre());
                aux.put("duracion", arreglo[i].getDuracion().toString());
                aux.put("url", arreglo[i].getUrl());
                aux.put("tipo", arreglo[i].getTipo().toString());
                aux.put("idAlbum", new DaoAlbum().listAll().get(arreglo[i].getIdAlbum() - 1).getNombre());
                lista.add(aux);
            }
        }
        return lista;
